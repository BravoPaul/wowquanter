...
def initialize(context):
    ...
    g.pd_stock_count = (datetime.strptime('2012-01-01', '%Y-%m-%d').date(), None)
    g.flag = 1
    g.buy_list = []
    g.bought_list = []
    g.sell_list = []
    ...
    run_monthly(buy_with_cash_weight, -1, time='14:00', reference_security=g.index_security)
...
def contition_2(context):
    ...
    if dt.month in [1, 4, 7, 10]:
        ...
        result_tmp = get_fund_portfolio_by_year(2013, 2022, dt_str, dt)
...
def buy_method_market(data_all, method_name, context, **kargs):
    def buy_custom_1(**kargs_inner):
        ...
        if dt_str_format in fund_release_date:
            ...
            data_one_date = data_o[data_o['end_date'] == dt_str_format]
            data_one_date = data_one_date[
                data_one_date['alpha_pct_stockCount'] >= kargs_inner.get('alpha_psc_thold_bot')]
            data_one_date = data_one_date[
                data_one_date['alpha_pct_stockCount'] <= kargs_inner.get('alpha_psc_thold_top')]
            ...
            data_one_date = data_one_date[data_one_date['code'].map(lambda x: x in g.buy_list)]
...
def main_buy(context):
    rank_method = {'sort_1': {'by': ['alpha_pct_stockCount', 'alpha_profit_month_3', 'alpha_turnOver_30'],
                   ...
                   'sort_3': {'by': ['alpha_profit_month_3', 'alpha_turnOver_30'],
                              'ascending': [True, True]}, 'N_3': 10, 'alpha_psc_thold_bot': 0.02,
                   'alpha_psc_thold_top': 1000000}
...
def buy_with_cash_equal(context):
    ...
    cash_equal = available_value / len(g.buy_list)

    dict_buy = {}

    for one_stock in g.buy_list:
        if context.portfolio.positions.get(one_stock) is not None:
            stock_info = context.portfolio.positions.get(one_stock)
            value_now = stock_info.price * stock_info.total_amount
        else:
            value_now = 0
        dict_buy[one_stock] = (cash_equal, value_now, cash_equal - value_now)

    dict_buy = dict(sorted(dict_buy.items(), key=lambda item: item[1][2]))

    for one_stock, value in dict_buy.items():
        result = order_target_value(one_stock, value[0])
        if result is None:
            print('ERROR(买入{}失败,买入量：{}'.format(one_stock, str(value[0])))
            if context.portfolio.positions.get(one_stock).total_amount > 0:
